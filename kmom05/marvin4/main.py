...
def main():
    ...
    while not stop:
        ...
        print("13) Search for countries")
        print("14) Emission data for years 1990, 2005, 2017")
        print("15) Get country data")
    ...
    #Gets acronyms
            ...
            inp = (input("Write a country name >>> "))
            try:
                print(f"Following countries were found: {emission_functions.search_country(inp)}")
            except ValueError:
                print("Country does not exist!")

        elif choice == "14":
            inp = input("Write country and dates seperated by , >>> ")
            reslist = inp.split(",")
            try:
                if len(reslist) == 2:
                    print(f"{reslist[0]}: {emission_functions.get_country_year_data_megaton(reslist[0], reslist[1])}")
                else:
                    str_res = emission_functions.get_country_change_for_years(reslist[0], reslist[1], reslist[2])
                    print(f"{reslist[0]}:{str_res}%")
            except ValueError:
                print("Wrong year!")

        elif choice == "15":
            inp = input("Write the country you want data about >>> ")
            resdata = emission_functions.get_country_data(inp)
            emission_functions.print_country_data(resdata)
